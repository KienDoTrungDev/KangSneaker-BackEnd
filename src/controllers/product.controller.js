...
const getFilterProducts = async (req, res, next) => {
  try {
    ...
    let dOptionQuery = helpers.convertObjectContainsRegex(
      JSON.parse(dOption),
    );

    for(const key in dOptionQuery){if(Object.keys(dOptionQuery[key]).length === 0 || dOptionQuery[key]===  '')delete dOptionQuery[key]}
    for(const key in pOptionQuery){if(Object.keys(pOptionQuery[key]).length === 0  || pOptionQuery[key]===  '')delete pOptionQuery[key]}
